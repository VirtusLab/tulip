 ...
 * (they're not in `result.assignments` to begin with). Matches assignment category ids to
 * `result.categories` the same way coverage verification does (./coverage.ts,
 * ./category-match.js) — normalized, not exact-string — so a change coverage already counted as
 * covered can't still silently fail to land under any category here.
 */
