...
- diagram kind — flag a sequence diagram whose subject is structure (modules, types, states,
  schema) rather than the order of calls, and any diagram that isn't a flowchart, sequence,
  class, state, or ER diagram.
...
- correctness — does every claim actually match the changes above? Flag anything invented,
  ...
  pseudocode, check that summary against the referenced real code and flag drift. Diagram nodes
  and edges are claims too — check that each import, call, relation, or transition drawn exists
  in the code.
