...
- top-down structure — does it open with the big picture (how the pieces fit end-to-end, with an
  orienting diagram where the shape warrants one) before drilling into individual changes? Flag
  ...
  only a sentence or two, and no diagram when the shape is trivial.
- diagram kind — is each diagram the kind that fits what changed? New modules, packages, or
  imports between them want a dependency graph; data moving through stages wants a data-flow
  diagram; new types want a class diagram; a state machine wants a state diagram; a database
  schema wants an ER diagram; a sequence diagram fits only when the order of calls is the point.
  Flag a sequence diagram whose subject is structure — modules, types, states, schema — rather
  than the order of calls, and a non-trivial structural change whose shape the prose doesn't
  carry and no diagram shows.
