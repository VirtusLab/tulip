...
Below are large changes. Each has a `changeId` and its diff lines. A change may have a
base side (removed lines, before the PR), a head side (added lines, after), or both (an
in-place rewrite); each side's lines are numbered on that side.
...
`boundaries` list (empty if it should not be split).
