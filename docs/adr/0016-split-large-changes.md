...
- **Candidate selection is deterministic:** a Change whose line count
  (`range.end - range.start + 1`) exceeds `config.limits.splitThreshold` (120). Both
  sides qualify, so large deletions split too. Sub-threshold changes pass through
  untouched.
...
- **Partition by construction.** Code, not the model, builds the partition. Split
  points are sanitized to integers in `(range.start, range.end]`, deduped and
  sorted; segments are `[start, p1-1], [p1, p2-1], …, [pk, end]`, each sub-Change's
  `lines` sliced at offset `segStart - range.start`, `side`/`path` inherited, id
  `${path}:${side}:${segStart}-${segEnd}`. The result tiles the original range
  exactly regardless of the model's output. An empty, invalid, or unknown-`changeId`
  response passes the change through whole.
...
- New: a `src/splitting/` module, a `split` prompt (`src/prompts/`, ADR 0006), and
  ...
  `config.models.changeSplitting`. No change to the `Change` model, classification,
  grouping, or rendering.

## References

- ADR 0005 (category identity by id) — the `path:side:start-end` change id
  sub-Changes reuse; ids stay code-assigned.
